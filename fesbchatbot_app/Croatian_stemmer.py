...
def transformiraj(pojavnica):
	transformacije=[e.strip().split('\t') for e in open('transformations.txt', encoding="utf8")]
...
def korjenuj(pojavnica):
	pravila=[re.compile(r'^('+osnova+')('+nastavak+r')$') for osnova, nastavak in [e.strip().split(' ') for e in open('fesbchatbot_app/rules.txt', encoding="utf8")]]
